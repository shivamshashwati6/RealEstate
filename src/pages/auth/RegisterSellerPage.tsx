...
import { Building, ShieldCheck, ArrowRight, Phone, Mail, FileText, CheckCircle2 } from 'lucide-react';
...
export const RegisterSellerPage: React.FC = () => {
  ...
  const handleSubmitDetails = (e: React.FormEvent) => {
    e.preventDefault();
    setStep('otp');
    addNotification('info', 'OTP Sent for Verification', `Enter demo OTP: 123456 sent to ${phone || email}`);
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-8 shadow-2xl space-y-6 animate-fadeIn">
        ...
        {step === 'details' ? (
          <form onSubmit={handleSubmitDetails} className="space-y-4 text-xs">
            ...
            >
              <span>Proceed to Agent Verification</span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </form>
        ) : (
          <form onSubmit={handleVerifyOtp} className="space-y-4 text-xs">
            <div className="bg-amber-950/60 p-4 rounded-2xl border border-amber-800 text-center text-xs text-amber-300 space-y-1">
              <span className="font-bold block">Agent Phone Verification</span>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-950 text-center text-xl font-bold tracking-widest text-white py-3 rounded-xl border border-slate-800 focus:border-amber-500 focus:outline-none"
              />
            </div>
            ...
            >
              Verify Agent OTP & Launch Studio
            </button>
          </form>
        )}
