...
export const SellerRegisterPage: React.FC = () => {
  ...
  const [accountType, setAccountType] = useState<'individual' | 'agency'>('agency');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    switchRole('seller');
    addNotification('success', 'Seller Account Activated!', 'Welcome to Seller Studio. Instant session created.');
    ...
    <div className="min-h-[85vh] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-8 shadow-2xl space-y-6 animate-fadeIn">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Seller Registration</h1>
          <p className="text-xs text-slate-400">List properties, receive visit requests & close deals</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          <div>
            <label className="block font-semibold text-slate-300 mb-1.5">Seller Account Type</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block font-semibold text-slate-300 mb-1">
                {accountType === 'agency' ? 'Agency / Brokerage Name' : 'Company / Business Name'}
              </label>
              <input
                type="text"
        ...
        <div className="text-center text-xs text-slate-400 pt-2 border-t border-slate-800">
          ...
          <Link to="/auth/seller/login" className="text-amber-400 font-semibold hover:underline">
