...
import { User, ArrowRight, AlertCircle, Home, Sparkles, Building2 } from 'lucide-react';
...
export const BuyerLoginPage: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Look up user by email or account selection
    ...
    switchRole('buyer');
    addNotification('success', 'Welcome Home Hunter!', 'Logged into your Buyer Dashboard.');
    ...
    <div className="min-h-[85vh] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-8 shadow-2xl space-y-6 relative overflow-hidden animate-fadeIn">
        ...
        <div className="text-center space-y-2 relative z-10">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] font-extrabold uppercase">
            <Sparkles className="w-3 h-3" /> Home Seeker Portal
          </div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Buyer Sign In</h1>
          <p className="text-xs text-slate-400">Access saved homes, tour schedules & seller chat</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4 text-xs relative z-10">
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Select Buyer Account / Email *</label>
            <select
              value={email}
              onChange={(e) => {
            ...
            >
              <option value="alex.buyer@example.com">Alex Johnson (Buyer)</option>
        ...
        <div className="text-center text-xs text-slate-400 pt-2 border-t border-slate-800 relative z-10">
          ...
          <Link to="/auth/buyer/register" className="text-emerald-400 font-semibold hover:underline">
            Register as a Buyer
          </Link>
        </div>
      </div>
