...
export const AuthPage: React.FC<AuthPageProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (authModalMode === 'login') {
      ...
      if (step === 'form') {
        setStep('otp');
        addNotification('info', 'OTP Verification Code Sent', 'Use demo OTP code: 123456');
      } else {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-6 relative animate-fadeIn">
        ...
        {step === 'otp' ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="bg-emerald-950/60 p-3 rounded-xl border border-emerald-800 text-xs text-emerald-300 text-center">
              We sent a 6-digit OTP verification code to <strong>{phone || email}</strong>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5 text-center">Enter 6-Digit OTP (Use: 123456)</label>
              <input
                type="text"
                maxLength={6}
                required
                ...
                className="w-full bg-slate-950 text-center tracking-widest text-lg font-bold text-white py-3 rounded-xl border border-slate-800 focus:border-emerald-500 focus:outline-none"
              />
            </div>
            <button
              type="submit"
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            ...
            >
              <span>{authModalMode === 'login' ? 'Sign In' : authModalMode === 'register' ? 'Proceed to OTP Verification' : 'Send Reset Link'}</span>
