...
export const RoleSwitcherBar: React.FC = () => {
  const { role, currentUserProfile, switchRole } = useAuthStore();
  const navigate = useNavigate();

  const roles: { role: 'buyer' | 'seller' | 'admin'; label: string; icon: React.ReactNode; color: string }[] = [
  ...
  const handleSwitch = (targetRole: 'buyer' | 'seller' | 'admin') => {
  ...
  return (
    <div className="bg-slate-950/90 border-b border-slate-800 text-xs py-1.5 px-4 sticky top-0 z-50 backdrop-blur-md flex flex-wrap items-center justify-between gap-2">
      <div className="flex items-center gap-2 text-slate-400">
        ...
        <span className="hidden sm:inline text-slate-400">Current User: <strong className="text-slate-200">{currentUserProfile?.name || 'Guest'}</strong> ({role?.toUpperCase() || 'GUEST'})</span>
      </div>
