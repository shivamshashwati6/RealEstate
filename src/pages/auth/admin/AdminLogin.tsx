...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // 4. DIRECT IMMEDIATE NAVIGATION to Admin Dashboard
    setLoading(false);
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-8 shadow-2xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">Admin Governance Login</h2>
          <p className="text-xs text-slate-400">Platform telemetry, listing queue & moderation</p>
        </div>
        ...
        <div className="pt-2 text-center text-xs text-slate-400 border-t border-slate-800 flex justify-between">
          <Link to="/auth/admin/register" className="hover:text-rose-400">
